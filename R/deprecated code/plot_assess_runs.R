...
model_pathv2 = here("ADMB_Models", "Exploratory_Models_v2")
...
grwth_vary_pl_rep <- readLines(here(model_pathv2, "Prop_Within_Growth_Vary_PartialLen", "tem.rep"))
grwth_vary_pl_sab_curr <- dget(here(model_pathv2, "Prop_Within_Growth_Vary_PartialLen", "tem.rdat")) 
grw_vary_pl_ctl_wts <- readLines(here(model_pathv2, "Prop_Within_Growth_Vary_PartialLen", "tem.ctl"))
grwth_sq_pl_rep <- readLines(here(model_pathv2, "Prop_Within_Growth_SQ_PartialLen", "tem.rep"))
grwth_sq_pl_sab_curr <- dget(here(model_pathv2, "Prop_Within_Growth_SQ_PartialLen", "tem.rdat")) 
grw_sq_pl_ctl_wts <- readLines(here(model_pathv2, "Prop_Within_Growth_SQ_PartialLen", "tem.ctl"))
# Extract out comps -------------------------------------------------------
## Fixed Gear Ages ---------------------------------------------------------
...
# Status-quo model partia length
# Get fixed gear ages
sq_pl_obs_ac_fish1_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$oac.fish1.f), type = "obs") %>% mutate(sex = "Female")
sq_pl_pred_ac_fish1_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$eac.fish1.f), type = "pred") %>% mutate(sex = "Female")
sq_pl_obs_ac_fish1_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$oac.fish1.m), type = "obs") %>% mutate(sex = "Male")
sq_pl_pred_ac_fish1_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$eac.fish1.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
sq_pl_ac_fish1 = rbind(sq_pl_obs_ac_fish1_f, sq_pl_pred_ac_fish1_f, sq_pl_obs_ac_fish1_m, sq_pl_pred_ac_fish1_m)
names(sq_pl_ac_fish1) = c("year", "age", "prop", "type", "sex")
sq_pl_ac_fish1$broodYear = sq_pl_ac_fish1$year - sq_pl_ac_fish1$age # create brood year to track cohort over time
sq_pl_ac_fish1 = sq_pl_ac_fish1 %>% mutate(broodYear = ifelse(age == 31, "31", broodYear), Model = "Growth SQ Partial Length") # make plus group consistent color

# Growth model vary partia length
# Get fixed gear ages
gv_pl_obs_ac_fish1_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$oac.fish1.f), type = "obs") %>% mutate(sex = "Female")
gv_pl_pred_ac_fish1_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$eac.fish1.f), type = "pred") %>% mutate(sex = "Female")
gv_pl_obs_ac_fish1_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$oac.fish1.m), type = "obs") %>% mutate(sex = "Male")
gv_pl_pred_ac_fish1_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$eac.fish1.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
gv_pl_ac_fish1 = rbind(gv_pl_pred_ac_fish1_m, gv_pl_obs_ac_fish1_m, gv_pl_pred_ac_fish1_f, gv_pl_obs_ac_fish1_f)
names(gv_pl_ac_fish1) = c("year", "age", "prop", "type", "sex")
gv_pl_ac_fish1$broodYear = sq_pl_ac_fish1$year - sq_pl_ac_fish1$age # create brood year to track cohort over time
gv_pl_ac_fish1 = gv_pl_ac_fish1 %>% mutate(broodYear = ifelse(age == 31, "31", broodYear), Model = "Growth Vary Partial Length") # make plus group consistent color
...
avg_sq_pl_ac_fish1 = sq_pl_ac_fish1 %>%
  group_by(age, type, sex, Model) %>% 
  summarize(prop = mean(prop)) 

avg_gv_pl_ac_fish1 = gv_pl_ac_fish1 %>%
  group_by(age, type, sex, Model) %>% 
  summarize(prop = mean(prop)) 

# Bind these together
all_fixed_gear_plot_df = rbind(gv_ac_fish1, sq_ac_fish1, gv_pl_ac_fish1,sq_pl_ac_fish1)
all_avg_fixed_gear_plot_df = rbind(avg_gv_ac_fish1, avg_sq_ac_fish1, avg_gv_pl_ac_fish1, avg_sq_pl_ac_fish1)
...
gv_pl_fish1f = get_osa_res(obs = grwth_vary_pl_sab_curr$oac.fish1.f, 
                           pred = grwth_vary_pl_sab_curr$eac.fish1.f, 
                           iss = rep(20, nrow(grwth_vary_pl_sab_curr$eac.fish1.f)), 
                           iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt fish1 age comp female iter")], " ")[[1]][1]), 
                           index = seq(3,31,1),
                           drop_bin = 1)

gv_pl_fish1f_plot = res_plot(data = gv_pl_fish1f[[1]], 
                             comp_type = "Age", res_type = "(Growth Vary Partial Lengths Fishery LL Age Comps Females)",
                             ymin = 2)

sq_pl_fish1f = get_osa_res(obs = grwth_sq_pl_sab_curr$oac.fish1.f, 
                           pred = grwth_sq_pl_sab_curr$eac.fish1.f, 
                           iss = rep(20, nrow(grwth_sq_pl_sab_curr$eac.fish1.f)), 
                           iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt fish1 age comp female iter")], " ")[[1]][1]), 
                           index = seq(3,31,1),
                           drop_bin = 1)

sq_pl_fish1f_plot = res_plot(data = sq_pl_fish1f[[1]], 
                             comp_type = "Age", res_type = "(Growth SQ Partial Lengths Fishery LL Age Comps Females)",
                             ymin = 2)
...
gv_pl_fish1m = get_osa_res(obs = grwth_vary_pl_sab_curr$oac.fish1.m, 
                           pred = grwth_vary_pl_sab_curr$eac.fish1.m, 
                           iss = rep(20, nrow(grwth_vary_pl_sab_curr$eac.fish1.m)), 
                           iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt fish1 age comp male iter")], " ")[[1]][1]), 
                           index = seq(3,31,1),
                           drop_bin = 1)

gv_pl_fish1m_plot = res_plot(data = gv_pl_fish1m[[1]], 
                             comp_type = "Age", res_type = "(Growth Vary Partial Lengths Fishery LL Age Comps Males)",
                             ymin = 2)

sq_pl_fish1m = get_osa_res(obs = grwth_sq_pl_sab_curr$oac.fish1.m, 
                           pred = grwth_sq_pl_sab_curr$eac.fish1.m, 
                           iss = rep(20, nrow(grwth_sq_pl_sab_curr$eac.fish1.m)), 
                           iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt fish1 age comp male iter")], " ")[[1]][1]), 
                           index = seq(3,31,1),
                           drop_bin = 1)

sq_pl_fish1m_plot = res_plot(data = sq_pl_fish1m[[1]], 
                             comp_type = "Age", res_type = "(Growth SQ Partial Lengths Fishery LL Age Comps Males)",
                             ymin = 2)

pdf(here("figs", "Model Comparison", "FixedGear_Age_Resid.pdf"), width = 15)
ggpubr::ggarrange(sq_fish1f_plot, gv_fish1f_plot, sq_pl_fish1f_plot, gv_pl_fish1f_plot)
ggpubr::ggarrange(sq_fish1m_plot, gv_fish1m_plot, sq_pl_fish1m_plot, gv_pl_fish1m_plot)
dev.off()
...
# Status-quo model partia length
# Get fixed gear lengths
sq_pl_obs_lc_fish1_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$olc.fish1.f), type = "obs") %>% mutate(sex = "Female")
sq_pl_pred_lc_fish1_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$elc.fish1.f), type = "pred") %>% mutate(sex = "Female")
sq_pl_obs_lc_fish1_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$olc.fish1.m), type = "obs") %>% mutate(sex = "Male")
sq_pl_pred_lc_fish1_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$elc.fish1.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
sq_pl_lc_fish1 = rbind(sq_pl_obs_lc_fish1_f, sq_pl_pred_lc_fish1_f, sq_pl_obs_lc_fish1_m, sq_pl_pred_lc_fish1_m)
names(sq_pl_lc_fish1) = c("year", "len", "prop", "type", "sex")
sq_pl_lc_fish1 = sq_pl_lc_fish1 %>% mutate(Model = "Growth SQ Partial Length") # make plus group consistent color

# Growth model vary partia length
# Get fixed gear lengths
gv_pl_obs_lc_fish1_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$olc.fish1.f), type = "obs") %>% mutate(sex = "Female")
gv_pl_pred_lc_fish1_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$elc.fish1.f), type = "pred") %>% mutate(sex = "Female")
gv_pl_obs_lc_fish1_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$olc.fish1.m), type = "obs") %>% mutate(sex = "Male")
gv_pl_pred_lc_fish1_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$elc.fish1.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
gv_pl_lc_fish1 = rbind(gv_pl_pred_lc_fish1_m, gv_pl_obs_lc_fish1_m, gv_pl_pred_lc_fish1_f, gv_pl_obs_lc_fish1_f)
names(gv_pl_lc_fish1) = c("year", "len", "prop", "type", "sex")
gv_pl_lc_fish1 = gv_pl_lc_fish1 %>% mutate(Model = "Growth Vary Partial Length") # make plus group consistent color
...
# plot average fits as well
avg_gv_pl_lc_fish1 = gv_pl_lc_fish1 %>%
  group_by(len, type, sex, Model) %>% 
  summarize(prop = mean(prop))

avg_sq_pl_lc_fish1 = sq_pl_lc_fish1 %>%
  group_by(len, type, sex, Model) %>% 
  summarize(prop = mean(prop)) 

# Bind these together
all_fixed_gear_plot_df = rbind(gv_lc_fish1, sq_lc_fish1, sq_pl_lc_fish1, gv_pl_lc_fish1)
all_avg_fixed_gear_plot_df = rbind(avg_gv_lc_fish1, avg_sq_lc_fish1, avg_sq_pl_lc_fish1, avg_gv_pl_lc_fish1)
...
gv_pl_lenfish1f = get_osa_res(obs = grwth_vary_pl_sab_curr$olc.fish1.f[-c(10:34),], 
                              pred = grwth_vary_pl_sab_curr$elc.fish1.f[-c(10:34),], 
                              iss = 20, 
                              iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt fish1 size comp female iter")], " ")[[1]][1]), 
                              index = seq(43,99,2),
                              drop_bin = 1)

gv_pl_lenfish1f_plot = res_plot(data = gv_pl_lenfish1f[[1]], 
                                comp_type = "Length", res_type = "(Growth Vary Partial Lengths Fishery LL size Comps Females)",
                                ymin = 43)

sq_pl_lenfish1f = get_osa_res(obs = grwth_sq_pl_sab_curr$olc.fish1.f[-c(10:34),], 
                              pred = grwth_sq_pl_sab_curr$elc.fish1.f[-c(10:34),], 
                              iss = 20, 
                              iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt fish1 size comp female iter")], " ")[[1]][1]), 
                              index = seq(43,99,2),
                              drop_bin = 1)

sq_pl_lenfish1f_plot = res_plot(data = sq_pl_lenfish1f[[1]], 
                                comp_type = "Length", res_type = "(Growth SQ Partial Lengths Fishery LL size Comps Females)",
                                ymin = 43)
...
gv_pl_fish1m = get_osa_res(obs = grwth_vary_pl_sab_curr$olc.fish1.m[-c(10:34),], 
                           pred = grwth_vary_pl_sab_curr$elc.fish1.m[-c(10:34),], 
                           iss = 20, 
                           iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt fish1 size comp male iter")], " ")[[1]][1]), 
                           index = seq(43,99,2),
                           drop_bin = 1)

gv_pl_fish1m_plot = res_plot(data = gv_pl_fish1m[[1]], 
                             comp_type = "size", res_type = "(Growth Vary Partial Lengths Fishery LL size Comps Males)",
                             ymin = 43)

sq_pl_fish1m = get_osa_res(obs = grwth_sq_pl_sab_curr$olc.fish1.m[-c(10:34),], 
                           pred = grwth_sq_pl_sab_curr$elc.fish1.m[-c(10:34),], 
                           iss = 20, 
                           iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt fish1 size comp male iter")], " ")[[1]][1]), 
                           index = seq(43,99,2),
                           drop_bin = 1)

sq_pl_fish1m_plot = res_plot(data = sq_pl_fish1m[[1]], 
                             comp_type = "size", res_type = "(Growth SQ Partial Lengths Fishery LL size Comps Males)",
                             ymin = 43)

pdf(here("figs", "Model Comparison", "FisheryLL_size_Resid.pdf"), width = 15)
ggpubr::ggarrange(sq_lenfish1f_plot, gv_lenfish1f_plot, sq_pl_lenfish1f_plot, gv_pl_lenfish1f_plot)
ggpubr::ggarrange(sq_fish1m_plot, gv_fish1m_plot, sq_pl_fish1m_plot, gv_pl_fish1m_plot)
dev.off()
...
# Status-quo model partia length
# Get trawl gear lengths
sq_pl_obs_lc_fish3_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$olc.fish3.f), type = "obs") %>% mutate(sex = "Female")
sq_pl_pred_lc_fish3_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$elc.fish3.f), type = "pred") %>% mutate(sex = "Female")
sq_pl_obs_lc_fish3_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$olc.fish3.m), type = "obs") %>% mutate(sex = "Male")
sq_pl_pred_lc_fish3_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$elc.fish3.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
sq_pl_lc_fish3 = rbind(sq_pl_obs_lc_fish3_f, sq_pl_pred_lc_fish3_f, sq_pl_obs_lc_fish3_m, sq_pl_pred_lc_fish3_m)
names(sq_pl_lc_fish3) = c("year", "len", "prop", "type", "sex")
sq_pl_lc_fish3 = sq_pl_lc_fish3 %>% mutate(Model = "Growth SQ Partial Length") # make plus group consistent color

# Growth model vary partia length
# Get trawl gear lengths
gv_pl_obs_lc_fish3_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$olc.fish3.f), type = "obs") %>% mutate(sex = "Female")
gv_pl_pred_lc_fish3_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$elc.fish3.f), type = "pred") %>% mutate(sex = "Female")
gv_pl_obs_lc_fish3_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$olc.fish3.m), type = "obs") %>% mutate(sex = "Male")
gv_pl_pred_lc_fish3_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$elc.fish3.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
gv_pl_lc_fish3 = rbind(gv_pl_pred_lc_fish3_m, gv_pl_obs_lc_fish3_m, gv_pl_pred_lc_fish3_f, gv_pl_obs_lc_fish3_f)
names(gv_pl_lc_fish3) = c("year", "len", "prop", "type", "sex")
gv_pl_lc_fish3 = gv_pl_lc_fish3 %>% mutate(Model = "Growth Vary Partial Length") # make plus group consistent color

# plot average fits as well
avg_gv_pl_lc_fish3 = gv_pl_lc_fish3 %>%
  group_by(len, type, sex, Model) %>% 
  summarize(prop = mean(prop))

avg_sq_pl_lc_fish3 = sq_pl_lc_fish3 %>%
  group_by(len, type, sex, Model) %>% 
  summarize(prop = mean(prop)) 
...
# Bind these together
all_trawl_gear_plot_df = rbind(gv_lc_fish3, sq_lc_fish3, gv_pl_lc_fish3, sq_pl_lc_fish3)
all_avg_trawl_gear_plot_df = rbind(avg_gv_lc_fish3, avg_sq_lc_fish3, avg_sq_pl_lc_fish3, avg_gv_pl_lc_fish3)
...
gv_pl_lenfish3f = get_osa_res(obs = grwth_vary_pl_sab_curr$olc.fish3.f, 
                              pred = grwth_vary_pl_sab_curr$elc.fish3.f, 
                              iss = rep(20, nrow(grwth_vary_pl_sab_curr$elc.fish3.f)), 
                              iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt fish 3 size comp female iter")], " ")[[1]][1]), 
                              index = seq(43,99,2),
                              drop_bin = 1)

gv_pl_lenfish3f_plot = res_plot(data = gv_pl_lenfish3f[[1]], 
                                comp_type = "Length", res_type = "(Growth Vary Partial Lengths Fishery Trawl size Comps Females)",
                                ymin = 43)

sq_pl_lenfish3f = get_osa_res(obs = grwth_sq_pl_sab_curr$olc.fish3.f, 
                              pred = grwth_sq_pl_sab_curr$elc.fish3.f, 
                              iss = rep(20, nrow(grwth_sq_pl_sab_curr$elc.fish3.f)), 
                              iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt fish 3 size comp female iter")], " ")[[1]][1]), 
                              index = seq(43,99,2),
                              drop_bin = 1)

sq_pl_lenfish3f_plot = res_plot(data = sq_pl_lenfish3f[[1]], 
                                comp_type = "Length", res_type = "(Growth SQ Partial Lengths Fishery Trawl size Comps Females)",
                                ymin = 43)
...
gv_pl_fish3m = get_osa_res(obs = grwth_vary_pl_sab_curr$olc.fish3.m, 
                           pred = grwth_vary_pl_sab_curr$elc.fish3.m, 
                           iss = rep(20, nrow(grwth_vary_pl_sab_curr$elc.fish3.m)), 
                           iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt fish 3 size comp male iter")], " ")[[1]][1]), 
                           index = seq(43,99,2),
                           drop_bin = 1)

gv_pl_fish3m_plot = res_plot(data = gv_pl_fish3m[[1]], 
                             comp_type = "size", res_type = "(Growth Vary Partial Lengths Fishery Trawl size Comps Males)",
                             ymin = 43)

sq_pl_fish3m = get_osa_res(obs = grwth_sq_pl_sab_curr$olc.fish3.m, 
                           pred = grwth_sq_pl_sab_curr$elc.fish3.m, 
                           iss = rep(20, nrow(grwth_sq_pl_sab_curr$elc.fish3.m)), 
                           iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt fish 3 size comp male iter")], " ")[[1]][1]), 
                           index = seq(43,99,2),
                           drop_bin = 1)

sq_pl_fish3m_plot = res_plot(data = sq_pl_fish3m[[1]], 
                             comp_type = "size", res_type = "(Growth SQ Partial Lengths Fishery Trawl size Comps Males)",
                             ymin = 43)

pdf(here("figs", "Model Comparison", "FisheryTrawl_size_Resid.pdf"), width = 15)
ggpubr::ggarrange(sq_lenfish3f_plot, gv_lenfish3f_plot, sq_pl_lenfish3f_plot, gv_pl_lenfish3f_plot)
ggpubr::ggarrange(sq_fish3m_plot, gv_fish3m_plot, sq_pl_fish3m_plot, gv_pl_fish3m_plot)
dev.off()
...
# Status-quo model partia length
# Get srv ll ages
sq_pl_obs_ac_srv1_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$oac.srv1.f), type = "obs") %>% mutate(sex = "Female")
sq_pl_pred_ac_srv1_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$eac.srv1.f), type = "pred") %>% mutate(sex = "Female")
sq_pl_obs_ac_srv1_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$oac.srv1.m), type = "obs") %>% mutate(sex = "Male")
sq_pl_pred_ac_srv1_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$eac.srv1.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
sq_pl_ac_srv1 = rbind(sq_pl_obs_ac_srv1_f, sq_pl_pred_ac_srv1_f, sq_pl_obs_ac_srv1_m, sq_pl_pred_ac_srv1_m)
names(sq_pl_ac_srv1) = c("year", "age", "prop", "type", "sex")
sq_pl_ac_srv1$broodYear = sq_pl_ac_srv1$year - sq_pl_ac_srv1$age # create brood year to track cohort over time
sq_pl_ac_srv1 = sq_pl_ac_srv1 %>% mutate(broodYear = ifelse(age == 31, "31", broodYear), Model = "Growth SQ Partial Length") # make plus group consistent color

# Growth model vary partia length
# Get srv ll ages
gv_pl_obs_ac_srv1_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$oac.srv1.f), type = "obs") %>% mutate(sex = "Female")
gv_pl_pred_ac_srv1_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$eac.srv1.f), type = "pred") %>% mutate(sex = "Female")
gv_pl_obs_ac_srv1_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$oac.srv1.m), type = "obs") %>% mutate(sex = "Male")
gv_pl_pred_ac_srv1_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$eac.srv1.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
gv_pl_ac_srv1 = rbind(gv_pl_pred_ac_srv1_m, gv_pl_obs_ac_srv1_m, gv_pl_pred_ac_srv1_f, gv_pl_obs_ac_srv1_f)
names(gv_pl_ac_srv1) = c("year", "age", "prop", "type", "sex")
gv_pl_ac_srv1$broodYear = sq_pl_ac_srv1$year - sq_pl_ac_srv1$age # create brood year to track cohort over time
gv_pl_ac_srv1 = gv_pl_ac_srv1 %>% mutate(broodYear = ifelse(age == 31, "31", broodYear), Model = "Growth Vary Partial Length") # make plus group consistent color
...
avg_sq_pl_ac_srv1 = sq_pl_ac_srv1 %>%
  group_by(age, type, sex, Model) %>% 
  summarize(prop = mean(prop)) 

avg_gv_pl_ac_srv1 = gv_pl_ac_srv1 %>%
  group_by(age, type, sex, Model) %>% 
  summarize(prop = mean(prop)) 

# Bind these together
all_srv_ll_plot_df = rbind(gv_ac_srv1, sq_ac_srv1, sq_pl_ac_srv1, gv_pl_ac_srv1)
all_avg_srv_ll_plot_df = rbind(avg_gv_ac_srv1, avg_sq_ac_srv1, avg_gv_pl_ac_srv1, avg_sq_pl_ac_srv1)
...
gv_pl_srv1f = get_osa_res(obs = grwth_vary_pl_sab_curr$oac.srv1.f, 
                          pred = grwth_vary_pl_sab_curr$eac.srv1.f, 
                          iss = rep(20, nrow(grwth_vary_pl_sab_curr$eac.srv1.f)), 
                          iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt surv1 age comp female iter")], " ")[[1]][1]), 
                          index = seq(3,31,1),
                          drop_bin = 1)

gv_pl_srv1f_plot = res_plot(data = gv_pl_srv1f[[1]], 
                            comp_type = "Age", res_type = "(Growth Vary Partial Lengths Survey LL Age Comps Females)",
                            ymin = 2)

sq_pl_srv1f = get_osa_res(obs = grwth_sq_pl_sab_curr$oac.srv1.f, 
                          pred = grwth_sq_pl_sab_curr$eac.srv1.f, 
                          iss = rep(20, nrow(grwth_sq_pl_sab_curr$eac.srv1.f)), 
                          iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt surv1 age comp female iter")], " ")[[1]][1]), 
                          index = seq(3,31,1),
                          drop_bin = 1)

sq_pl_srv1f_plot = res_plot(data = sq_pl_srv1f[[1]], 
                            comp_type = "Age", res_type = "(Growth SQ Partial Lengths Survey LL Age Comps Females)",
                            ymin = 2)
...
gv_pl_srv1m = get_osa_res(obs = grwth_vary_pl_sab_curr$oac.srv1.m, 
                          pred = grwth_vary_pl_sab_curr$eac.srv1.m, 
                          iss = rep(20, nrow(grwth_vary_pl_sab_curr$eac.srv1.m)), 
                          iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt surv1 age comp male iter")], " ")[[1]][1]), 
                          index = seq(3,31,1),
                          drop_bin = 1)

gv_pl_srv1m_plot = res_plot(data = gv_pl_srv1m[[1]], 
                            comp_type = "Age", res_type = "(Growth Vary Partial Lengths Survey LL Age Comps Males)",
                            ymin = 2)

sq_pl_srv1m = get_osa_res(obs = grwth_sq_pl_sab_curr$oac.srv1.m, 
                          pred = grwth_sq_pl_sab_curr$eac.srv1.m, 
                          iss = rep(20, nrow(grwth_sq_pl_sab_curr$eac.srv1.m)), 
                          iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt surv1 age comp male iter")], " ")[[1]][1]), 
                          index = seq(3,31,1),
                          drop_bin = 1)

sq_pl_srv1m_plot = res_plot(data = sq_pl_srv1m[[1]], 
                            comp_type = "Age", res_type = "(Growth SQ Partial Lengths Survey LL Age Comps Males)",
                            ymin = 2)

pdf(here("figs", "Model Comparison", "SurveyLL_Age_Resid.pdf"), width = 15)
ggpubr::ggarrange(sq_srv1f_plot, gv_srv1f_plot, sq_pl_srv1f_plot, gv_pl_srv1f_plot)
ggpubr::ggarrange(sq_srv1m_plot, gv_srv1m_plot, sq_pl_srv1m_plot, gv_pl_srv1m_plot)
dev.off()
...
# Status-quo model partia length
# Get srv ll lengths
sq_pl_obs_lc_srv1_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$olc.srv1.f), type = "obs") %>% mutate(sex = "Female")
sq_pl_pred_lc_srv1_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$elc.srv1.f), type = "pred") %>% mutate(sex = "Female")
sq_pl_obs_lc_srv1_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$olc.srv1.m), type = "obs") %>% mutate(sex = "Male")
sq_pl_pred_lc_srv1_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$elc.srv1.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
sq_pl_lc_srv1 = rbind(sq_pl_obs_lc_srv1_f, sq_pl_pred_lc_srv1_f, sq_pl_obs_lc_srv1_m, sq_pl_pred_lc_srv1_m)
names(sq_pl_lc_srv1) = c("year", "len", "prop", "type", "sex")
sq_pl_lc_srv1 = sq_pl_lc_srv1 %>% mutate(Model = "Growth SQ Partial Length") # make plus group consistent color

# Growth model vary partia length
# Get srv ll lengths
gv_pl_obs_lc_srv1_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$olc.srv1.f), type = "obs") %>% mutate(sex = "Female")
gv_pl_pred_lc_srv1_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$elc.srv1.f), type = "pred") %>% mutate(sex = "Female")
gv_pl_obs_lc_srv1_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$olc.srv1.m), type = "obs") %>% mutate(sex = "Male")
gv_pl_pred_lc_srv1_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$elc.srv1.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
gv_pl_lc_srv1 = rbind(gv_pl_pred_lc_srv1_m, gv_pl_obs_lc_srv1_m, gv_pl_pred_lc_srv1_f, gv_pl_obs_lc_srv1_f)
names(gv_pl_lc_srv1) = c("year", "len", "prop", "type", "sex")
gv_pl_lc_srv1 = gv_pl_lc_srv1 %>% mutate(Model = "Growth Vary Partial Length") # make plus group consistent color

# plot average fits as well
avg_gv_pl_lc_srv1 = gv_pl_lc_srv1 %>%
  group_by(len, type, sex, Model) %>% 
  summarize(prop = mean(prop))

avg_sq_pl_lc_srv1 = sq_pl_lc_srv1 %>%
  group_by(len, type, sex, Model) %>% 
  summarize(prop = mean(prop)) 
...
# Bind these together
all_srv_ll_plot_df = rbind(gv_lc_srv1, sq_lc_srv1, gv_pl_lc_srv1, sq_pl_lc_srv1)
all_avg_srv_ll_plot_df = rbind(avg_gv_lc_srv1, avg_sq_lc_srv1,avg_gv_pl_lc_srv1, avg_sq_pl_lc_srv1)
...
gv_pl_lensrv1f = get_osa_res(obs = grwth_vary_pl_sab_curr$olc.srv1.f[-c(10:33),], 
                             pred = grwth_vary_pl_sab_curr$elc.srv1.f[-c(10:33),], 
                             iss = 20, 
                             iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt surv1 size comp female iter")], " ")[[1]][1]), 
                             index = seq(43,99,2),
                             drop_bin = 1)

gv_pl_lensrv1f_plot = res_plot(data = gv_pl_lensrv1f[[1]], 
                               comp_type = "Length", res_type = "(Growth Vary Partial Lengths Survey LL size Comps Females)",
                               ymin = 43)

sq_pl_lensrv1f = get_osa_res(obs = grwth_sq_pl_sab_curr$olc.srv1.f[-c(10:33),], 
                             pred = grwth_sq_pl_sab_curr$elc.srv1.f[-c(10:33),], 
                             iss = 20, 
                             iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt surv1 size comp female iter")], " ")[[1]][1]), 
                             index = seq(43,99,2),
                             drop_bin = 1)

sq_pl_lensrv1f_plot = res_plot(data = sq_pl_lensrv1f[[1]], 
                               comp_type = "Length", res_type = "(Growth SQ Partial Lengths Survey LL size Comps Females)",
                               ymin = 43)
...
gv_pl_srv1m = get_osa_res(obs = grwth_vary_pl_sab_curr$olc.srv1.m[-c(10:33),], 
                          pred = grwth_vary_pl_sab_curr$elc.srv1.m[-c(10:33),], 
                          iss = 20, 
                          iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt surv1 size comp male iter")], " ")[[1]][1]), 
                          index = seq(43,99,2),
                          drop_bin = 1)

gv_pl_srv1m_plot = res_plot(data = gv_pl_srv1m[[1]], 
                            comp_type = "size", res_type = "(Growth Vary Partial Lengths Survey LL size Comps Males)",
                            ymin = 43)

sq_pl_srv1m = get_osa_res(obs = grwth_sq_pl_sab_curr$olc.srv1.m[-c(10:33),], 
                          pred = grwth_sq_pl_sab_curr$elc.srv1.m[-c(10:33),], 
                          iss = 20, 
                          iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt surv1 size comp male iter")], " ")[[1]][1]), 
                          index = seq(43,99,2),
                          drop_bin = 1)

sq_pl_srv1m_plot = res_plot(data = sq_pl_srv1m[[1]], 
                            comp_type = "size", res_type = "(Growth SQ Partial Lengths Survey LL size Comps Males)",
                            ymin = 43)

pdf(here("figs", "Model Comparison", "SurveyLL_size_Resid.pdf"), width = 15)
ggpubr::ggarrange(sq_lensrv1f_plot, gv_lensrv1f_plot, sq_pl_lensrv1f_plot, gv_pl_lensrv1f_plot)
ggpubr::ggarrange(sq_srv1m_plot, gv_srv1m_plot, sq_pl_srv1m_plot, gv_pl_srv1m_plot)
dev.off()
...
# Status-quo model partia length
# Get srv ll lengths
sq_pl_obs_lc_srv7_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$olc.srv7.f), type = "obs") %>% mutate(sex = "Female")
sq_pl_pred_lc_srv7_f = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$elc.srv7.f), type = "pred") %>% mutate(sex = "Female")
sq_pl_obs_lc_srv7_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$olc.srv7.m), type = "obs") %>% mutate(sex = "Male")
sq_pl_pred_lc_srv7_m = data.frame(reshape2::melt(grwth_sq_pl_sab_curr$elc.srv7.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
sq_pl_lc_srv7 = rbind(sq_pl_obs_lc_srv7_f, sq_pl_pred_lc_srv7_f, sq_pl_obs_lc_srv7_m, sq_pl_pred_lc_srv7_m)
names(sq_pl_lc_srv7) = c("year", "len", "prop", "type", "sex")
sq_pl_lc_srv7 = sq_pl_lc_srv7 %>% mutate(Model = "Growth SQ Partial Length") # make plus group consistent color

# Growth model vary partia length
# Get srv ll lengths
gv_pl_obs_lc_srv7_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$olc.srv7.f), type = "obs") %>% mutate(sex = "Female")
gv_pl_pred_lc_srv7_f = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$elc.srv7.f), type = "pred") %>% mutate(sex = "Female")
gv_pl_obs_lc_srv7_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$olc.srv7.m), type = "obs") %>% mutate(sex = "Male")
gv_pl_pred_lc_srv7_m = data.frame(reshape2::melt(grwth_vary_pl_sab_curr$elc.srv7.m), type = "pred") %>% mutate(sex = "Male")

# Put these into a dataframe
gv_pl_lc_srv7 = rbind(gv_pl_pred_lc_srv7_m, gv_pl_obs_lc_srv7_m, gv_pl_pred_lc_srv7_f, gv_pl_obs_lc_srv7_f)
names(gv_pl_lc_srv7) = c("year", "len", "prop", "type", "sex")
gv_pl_lc_srv7 = gv_pl_lc_srv7 %>% mutate(Model = "Growth Vary Partial Length") # make plus group consistent color

# plot average fits as well
avg_gv_pl_lc_srv7 = gv_pl_lc_srv7 %>%
  group_by(len, type, sex, Model) %>% 
  summarize(prop = mean(prop))

avg_sq_pl_lc_srv7 = sq_pl_lc_srv7 %>%
  group_by(len, type, sex, Model) %>% 
  summarize(prop = mean(prop)) 
...
# Bind these together
all_srv_trawl_plot_df = rbind(gv_lc_srv7, sq_lc_srv7, gv_pl_lc_srv7, sq_pl_lc_srv7)
all_avg_srv_trawl_plot_df = rbind(avg_gv_lc_srv7, avg_sq_lc_srv7,avg_gv_pl_lc_srv7, avg_sq_pl_lc_srv7)
...
gv_pl_lensrv7f = get_osa_res(obs = grwth_vary_pl_sab_curr$olc.srv7.f, 
                             pred = grwth_vary_pl_sab_curr$elc.srv7.f, 
                             iss = rep(20, nrow(grwth_vary_pl_sab_curr$elc.srv7.f)), 
                             iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt srv7 size comp female iter")], " ")[[1]][1]), 
                             index = seq(43,99,2),
                             drop_bin = 1)

gv_pl_lensrv7f_plot = res_plot(data = gv_pl_lensrv7f[[1]], 
                               comp_type = "Length", res_type = "(Growth Vary Partial Lengths Survey Trawl size Comps Females)",
                               ymin = 43)

sq_pl_lensrv7f = get_osa_res(obs = grwth_sq_pl_sab_curr$olc.srv7.f, 
                             pred = grwth_sq_pl_sab_curr$elc.srv7.f, 
                             iss = rep(20, nrow(grwth_sq_pl_sab_curr$elc.srv7.f)), 
                             iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt srv7 size comp female iter")], " ")[[1]][1]), 
                             index = seq(43,99,2),
                             drop_bin = 1)

sq_pl_lensrv7f_plot = res_plot(data = sq_pl_lensrv7f[[1]], 
                               comp_type = "Length", res_type = "(Growth SQ Partial Lengths Survey Trawl size Comps Females)",
                               ymin = 43)
...
gv_pl_srv7m = get_osa_res(obs = grwth_vary_pl_sab_curr$olc.srv7.m, 
                          pred = grwth_vary_pl_sab_curr$elc.srv7.m, 
                          iss = rep(20, nrow(grwth_vary_pl_sab_curr$elc.srv7.m)), 
                          iter_wt = as.numeric(strsplit(grw_vary_pl_ctl_wts[str_detect(grw_vary_pl_ctl_wts, "#wt srv7 size comp male iter")], " ")[[1]][1]), 
                          index = seq(43,99,2),
                          drop_bin = 1)

gv_pl_srv7m_plot = res_plot(data = gv_pl_srv7m[[1]], 
                            comp_type = "size", res_type = "(Growth Vary Partial Lengths Survey Trawl size Comps Males)",
                            ymin = 43)

sq_pl_srv7m = get_osa_res(obs = grwth_sq_pl_sab_curr$olc.srv7.m, 
                          pred = grwth_sq_pl_sab_curr$elc.srv7.m, 
                          iss = rep(20, nrow(grwth_sq_pl_sab_curr$elc.srv7.m)), 
                          iter_wt = as.numeric(strsplit(grw_sq_pl_ctl_wts[str_detect(grw_sq_pl_ctl_wts, "#wt srv7 size comp male iter")], " ")[[1]][1]), 
                          index = seq(43,99,2),
                          drop_bin = 1)

sq_pl_srv7m_plot = res_plot(data = sq_pl_srv7m[[1]], 
                            comp_type = "size", res_type = "(Growth SQ Partial Lengths Survey Trawl size Comps Males)",
                            ymin = 43)

pdf(here("figs", "Model Comparison", "SurveyTrawl_size_Resid.pdf"), width = 15)
ggpubr::ggarrange(sq_lensrv7f_plot, gv_lensrv7f_plot, sq_pl_lensrv7f_plot, gv_pl_lensrv7f_plot)
ggpubr::ggarrange(sq_srv7m_plot, gv_srv7m_plot, sq_pl_srv7m_plot, gv_pl_srv7m_plot)
dev.off()
...
gvpl_par <- R2admb::read_pars(fn = here(model_pathv2, "Prop_Within_Growth_Vary_PartialLen","tem"))
sqpl_par <- R2admb::read_pars(fn = here(model_pathv2, "Prop_Within_Growth_SQ_PartialLen","tem"))
...
gvpl_ssb <- par_coeff(gvpl_par, "ssbsd", type = "Growth Vary Partial Length") %>% mutate(Year = 1960:2023)
sqpl_ssb <- par_coeff(sqpl_par, "ssbsd", type = "Growth SQ Partial Length") %>% mutate(Year = 1960:2023)
...
gvpl_ssb_proj <- par_coeff(gvpl_par, "spawn_biom_proj", type = "Growth Vary Partial Length") %>% mutate(Year = 2024:2038)
sqpl_ssb_proj <- par_coeff(sqpl_par, "spawn_biom_proj", type = "Growth SQ Partial Length") %>% mutate(Year = 2024:2038)

# Get b40 as well
gv_b40 <- par_coeff(gv_par, "B40", type = "Growth Vary") 
sq_b40 <- par_coeff(sq_par, "B40", type = "Growth SQ") 
gvpl_b40 <- par_coeff(gvpl_par, "B40", type = "Growth Vary Partial Length") 
sqpl_b40 <- par_coeff(sqpl_par, "B40", type = "Growth SQ Partial Length") 
b40_df = rbind(gv_b40, sq_b40, gvpl_b40, sqpl_b40)

# residual munging
ssb_df = rbind(gv_ssb, sq_ssb, gv_ssb_proj, sq_ssb_proj, gvpl_ssb_proj, sqpl_ssb_proj,
               gvpl_ssb, sqpl_ssb) %>% 
...
gvpl_rec <- par_coeff(gvpl_par, "pred_rec", type = "Growth Vary Partial Length") %>% mutate(Year = 1960:2023)
sqpl_rec <- par_coeff(sqpl_par, "pred_rec", type = "Growth SQ Partial Length") %>%  mutate(Year = 1960:2023)
...
gvpl_meanrec <- par_coeff(gvpl_par, "log_mean_rec", type = "Growth Vary Partial Length") 
sqpl_meanrec <- par_coeff(sqpl_par, "log_mean_rec", type = "Growth SQ Partial Length") 

meanrec_df = rbind(gv_meanrec, sq_meanrec, gvpl_meanrec, sqpl_meanrec)
rec_df = rbind(gv_rec, sq_rec,gvpl_rec,sqpl_rec ) %>%  mutate(downr = ifelse(downr <=0, 0, downr)) %>% rename(rec = coeff)
...
ggplot(rec_df %>% filter(Year != 2023), aes(x = Year - 2, y = rec, ymin = downr, ymax = upr, fill = type)) +
  ...
  geom_point(rec_df %>% filter(Year >= 2014 & Year != 2023), mapping =  aes(color = type), size = 3, alpha = 0.65) +
  geom_hline(aes(yintercept = meanrec, color = type), lty = 2, size = 1.3) +
  theme_reg() +
  theme(legend.position = "top") +
  facet_wrap(~Model_Comps) +
  ylim(0, NA) +
  labs(x = "Year", y = "Age-2 Recruitment (millions)", color = "Model", fill = "Model")
ggplot(rec_df %>% filter(Year != 2023), aes(x = Year - 2, y = rec, ymin = downr, ymax = upr, fill = type)) +
  geom_line(aes(color = type), size = 1.2) +
  geom_point(aes(color = type), size = 3, alpha = 0.65) +
  theme_reg() +
  theme(legend.position = "top") +
  ylim(0, NA) +
  labs(x = "Year", y = "Age-2 Recruitment (millions)", color = "Model", fill = "Model")
...
for(i in 1:length(selex_names)) {
  ...
  gv_sel <- par_coeff(gv_par, selex_names[i], type = "Growth Vary") %>% 
    mutate(Age = 2:31, sel = selex_names[i]) %>% 
    mutate(downr = ifelse(downr <=0, 0, downr), upr = ifelse(upr >= 1, 1, upr)) 
  # status-quo model partial length
  sqpl_sel <- par_coeff(sqpl_par, selex_names[i], type = "Growth SQ Partial Length") %>% 
    mutate(Age = 2:31, sel = selex_names[i]) %>% 
    mutate(downr = ifelse(downr <=0, 0, downr), upr = ifelse(upr >= 1, 1, upr)) 
  # time-varying model partial length
  gvpl_sel <- par_coeff(gvpl_par, selex_names[i], type = "Growth Vary Partial Length") %>% 
    mutate(Age = 2:31, sel = selex_names[i]) %>% 
    mutate(downr = ifelse(downr <=0, 0, downr), upr = ifelse(upr >= 1, 1, upr)) 
  all_sel_df = rbind(sq_sel, gv_sel, gvpl_sel, sqpl_sel, all_sel_df)
} # end i
...
gvpl_abc <- par_coeff(gvpl_par, "ABC", type = "Growth Vary Partial Length") 
sqpl_abc <- par_coeff(sqpl_par, "ABC", type = "Growth SQ Partial Length")
abc_df = rbind(gv_abc, sq_abc, gvpl_abc, sqpl_abc)
...
gvpl_idx_df = data.frame()
gvpl_idx_list = grwth_vary_pl_sab_curr[str_detect(names(grwth_vary_pl_sab_curr), "obssrv")]
sqpl_idx_df = data.frame()
sqpl_idx_list = grwth_sq_pl_sab_curr[str_detect(names(grwth_sq_pl_sab_curr), "obssrv")]
...
for(i in 1:length(gv_idx_list)) {
  ...
  # extract out index dataframe (just extracting out components from a list) - growth vary model
  gvpl_idx_tmp = data.frame(year = as.numeric(rownames(gvpl_idx_list[[i]])), obs = gvpl_idx_list[[i]][[1]],
                          lci = gvpl_idx_list[[i]][[2]], uci = gvpl_idx_list[[i]][[3]],
                          pred = gvpl_idx_list[[i]][[4]], type = idx_names[i], Model = "Growth Vary Partial Length")
  # extract out index dataframe (just extracting out components from a list) - sq model
  sqpl_idx_tmp = data.frame(year = as.numeric(rownames(sqpl_idx_list[[i]])), obs = sqpl_idx_list[[i]][[1]],
                          lci = sqpl_idx_list[[i]][[2]], uci = sqpl_idx_list[[i]][[3]],
                          pred = sqpl_idx_list[[i]][[4]], type = idx_names[i], Model = "Growth SQ Partial Length")
  # bind together
  gv_idx_df = rbind(gv_idx_df, gv_idx_tmp)
  sq_idx_df = rbind(sq_idx_df, sq_idx_tmp)
  gvpl_idx_df = rbind(gvpl_idx_df, gvpl_idx_tmp)
  sqpl_idx_df = rbind(sqpl_idx_df, sqpl_idx_tmp)
} # end i loop

idx_df = rbind(gv_idx_df, sq_idx_df, sqpl_idx_df, gvpl_idx_df)
