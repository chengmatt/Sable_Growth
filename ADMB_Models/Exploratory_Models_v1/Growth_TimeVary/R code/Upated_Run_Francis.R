...
path<-here()
pathM<-paste(path,"/Growth_TimeVary",sep="")
...
ESS_init<-ESS_init[-c(1:yrs_JPLLFS)]
...
for (i in 1:ITERS){
  ...
  wt_LLFS<-1/var(w_denom)
  ESS_iter_LLFS[i+1,1]<-wt_LLFS
  ESS_iter_LLFS[i]<-sum((ESS_iter_LLFS[i+1,1]-ESS_iter_LLFS[i,1])^2)
