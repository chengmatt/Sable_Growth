...
// Read data from the control file
  ...
  init_int    growth_model_switch   //switch for running weight at age models (==0; status-quo time block), (==1; time-varying 3dar1)
...
// ###########################################################################################################################
 ...
 //############### for iterative reweighting runs
  ...
  init_number wt_fish1_size_iter          // Weight for fishery size compositions
  init_number wt_srv1_size_iter          // Weight for survey size compostiions
  init_number wt_srv2_size_iter          // Weight for survey size compostiions
  init_number wt_fish3_size_iter          // Weight for fishery size compositions
  init_number wt_fish2_size_iter          // weight for fishery size compositions
  init_number wt_srv7_age_iter            // weight for survey age comps
  init_number wt_srv7_size_iter          // Weight for fishery size compositions
 //########################################################
...
// Natural mortality, proportion mature and weight at age
  ...
  init_matrix   wt_tv_m(styr,endyr,1,nages); // time-varying males
  init_matrix   wt_tv_f(styr,endyr,1,nages); // time-varying males
  // Observed catches
...
// Survey biomass estimates
 ...
 !! cout<<"Number of trawl years:"<<nyrs_srv7<<endl;
  ...
  init_matrix   oac_fish1(1,nyrs_fish1_age,1,nages*2)   // the actual year by year age comps (proportions across ages), females then males
  ...
  init_matrix   oac_srv1(1,nyrs_srv1_age,1,nages*2)   // the year by year age survey age comps
  ...
  init_matrix   oac_srv2(1,nyrs_srv2_age,1,nages*2)   // the year by year age survey age comps
...
// Fishery size composition data
  ...
  init_matrix   osc_fish1(1,nyrs_fish1_size,1,nlenbins*2)// year by year fishery size comps (proportions across sexes females, then males)
  ...
  init_matrix   osc_fish3(1,nyrs_fish3_size,1,nlenbins*2)// year by year fishery size comps proporitonss across sexes, females then males
...
// Survey size composition data
  ...
  init_matrix   osc_srv1(1,nyrs_srv1_size,1,nlenbins*2)//year by year size comps (proportions across, females then males)
  ...
  init_matrix   osc_srv2(1,nyrs_srv2_size,1,nlenbins*2)//year by year size comps
  ...
  init_matrix   osc_srv7(1,nyrs_srv7_size,1,nlenbins*2)//year by year size comps
...
// Ageing error transition matrix:  proportion at reader age given true age
  ...
  init_3darray  sizeage_tv_m(styr,endyr,1,nages,1,nlenbins); // size-age matrix for 3dar1 laa process
  init_3darray  sizeage_tv_f(styr,endyr,1,nages,1,nlenbins); // size-age matrix for 3dar1 laa process
...
//   First step is to ensure that the data are expressed as proportions
  ...
  {
   osc_fish1(i)/=sum(osc_fish1(i));
   offset(3) -= nsamples_fish1_size(i)*((osc_fish1(i) + 0.001)*log(osc_fish1(i) + 0.001));
  }
  ...
  {
   osc_fish3(i)/=sum(osc_fish3(i));
   offset(6) -= nsamples_fish3_size(i)*((osc_fish3(i) + 0.001)*log(osc_fish3(i) + 0.001));
     }
  ...
  for (i=1; i<=nyrs_srv1_size; i++)
  {
   ...
   offset(9) -= nsamples_srv1_size(i)*((osc_srv1(i) + 0.001)*log(osc_srv1(i) + 0.001));
  }
  for (i=1; i<=nyrs_srv2_size; i++)
  {
   osc_srv2(i)/=sum(osc_srv2(i));
   offset(11) -= nsamples_srv2_size(i)*((osc_srv2(i) + 0.001)*log(osc_srv2(i) + 0.001));
  }
  for (i=1; i<=nyrs_srv7_size; i++)
  {
   osc_srv7(i)/=sum(osc_srv7(i));
   offset(13) -= nsamples_srv7_size(i)*((osc_srv7(i) + 0.001)*log(osc_srv7(i) + 0.001));
  }
...
// ########################################################################################################
 // steepness and R0 not estimated in base model, par have neg phases
  ...
  init_bounded_number    logit_sexRatio(-10,10,2);
// ########################################################################################################
...
// ########################################################################################################
 ...
 // ########################################################################################################
  ...
  init_bounded_number   log_a50_fish1_f(-5,5,ph_fish_sel_delt_alt);                 // age at 50% selection                                                   
  init_bounded_number   log_delta_fish1_f(-5,3,ph_fish_sel_delt_alt);                 // age between 50% selection and 95% selection....
  ...
  sdreport_vector        fish1_sel_f(1,nages);                          // vectory of fishery selectivty parameters on arithmetic scale
  ...
  init_bounded_number   log_a50_fish1_m(-5,5,ph_fish_sel_delt_alt);                 // age at 50% selection                                                   
  init_bounded_number   log_delta_fish1_m(-5,3,ph_fish_sel_delt_alt);                 // age between 50% selection and 95% selection....
  ...
  sdreport_vector        fish1_sel_m(1,nages);                          // vectory of fishery selectivty parameters on arithmetic scale
  ...
  init_bounded_number       log_a50_fish2(-1,5,ph_fish_sel_delt_alt);                 // age at 50% selection                                                   
  ...
  sdreport_vector        fish3_sel_f(1,nages);                          // vectory of fishery selectivty parameters on arithmetic scale
  ...
  sdreport_vector        fish3_sel_m(1,nages);                          // vectory of fishery selectivty parameters on arithmetic scale
  ...
  sdreport_vector        fish4_sel_f(1,nages);                          // vectory of fishery selectivty parameters on arithmetic scale
  ...
  sdreport_vector        fish4_sel_m(1,nages);                          // vectory of fishery selectivty parameters on arithmetic scale
  ...
  sdreport_vector        fish5_sel_f(1,nages);                          // vectory of fishery selectivty parameters on arithmetic scale
  ...
  sdreport_vector        fish5_sel_m(1,nages);                          // vectory of fishery selectivty parameters on arithmetic scale
...
// ########################################################################################################
  ...
  sdreport_vector        srv1_sel_f(1,nages);                  // vectory of survey selectivty parameters on arithmetic scale
  ...
  sdreport_vector        srv1_sel_m(1,nages);                  // vectory of survey selectivty parameters on arithmetic scale
  ...
  init_bounded_number   log_delta_srv2_f(-5,4,ph_srv2_sel);                 // age between 50% selection and 95% selection....
  ...
  sdreport_vector        srv2_sel_f(1,nages);                  // vectory of survey selectivty parameters on arithmetic scale
  ...
  init_bounded_number   log_delta_srv2_m(-5,4,ph_srv2_sel);                 // age between 50% selection and 95% selection....
  ...
  sdreport_vector        srv2_sel_m(1,nages);                  // vectory of survey selectivty parameters on arithmetic scale
  ...
  sdreport_vector        srv7_sel_f(1,nages);                           // vectory of fishery selectivty parameters on arithmetic scale
  ...
  sdreport_vector        srv7_sel_m(1,nages);                           // vectory of fishery selectivty parameters on arithmetic scale
  ...
  init_bounded_number   log_delta_srv10_f(-5,4,ph_LL_block2);                 // age between 50% selection and 95% selection....
  ...
  sdreport_vector        srv10_sel_f(1,nages);                  // vectory of survey selectivty parameters on arithmetic scale
  ...
  init_bounded_number   log_delta_srv10_m(-5,4,ph_LL_block2);                 // age between 50% selection and 95% selection....
  ...
  sdreport_vector        srv10_sel_m(1,nages);                  // vectory of survey selectivty parameters on arithmetic scale
...
// Catch at age
  ...
  matrix                eac_fish1(1,nyrs_fish1_age,1,nages*2)        // Expected proportion at age in fish (proporitons across ages)
  matrix                eac_srv1(1,nyrs_srv1_age,1,nages*2)        // Expected proportion at age in survey
  matrix                eac_srv2(1,nyrs_srv2_age,1,nages*2)        // Expected proportion at age in survey
  matrix                esc_fish1(1,nyrs_fish1_size,1,nlenbins*2)    // Expected proportion at size in fishery
  matrix                esc_fish2(1,nyrs_fish2_size,1,nlenbins)    // Expected proportion at size in fishery
  matrix                esc_fish3(1,nyrs_fish3_size,1,nlenbins*2)    // Expected proportion at size in fishery
  matrix                esc_fish4(1,nyrs_fish4_size,1,nlenbins)    // Expected proportion at size in fishery
  matrix                esc_srv1(1,nyrs_srv1_size,1,nlenbins*2)    // Expected proportion at size in survey
  matrix                esc_srv2(1,nyrs_srv2_size,1,nlenbins*2)    // Expected proportion at size in survey
  matrix                esc_srv7(1,nyrs_srv7_size,1,nlenbins*2)    // Expected proportion at size in survey
...
// Define an objective function
  ...
  vector        sexRatio(1,2); // sex ratio parameter
  number        fratio;
//  number        B40;
  number        ABC3;
...
//////////////////////////////////////////
 ...
 PROCEDURE_SECTION
  ...
  sexRatio(1) = 0 + (1 - 0) * (1 / (1 + exp(-logit_sexRatio)));
  sexRatio(2) = 1 - sexRatio(1);
...
FUNCTION Get_Bzero
  ...
  natage_f(styr)  = natagetmp(styr)*sexRatio(1); // OjO
  natage_m(styr)  = natagetmp(styr)*sexRatio(2); // OjO
...
// ###################################################################################################
 ...
 // All fishery fleets have their own sex specific a50
  a50_fish1_f=mfexp(log_a50_fish4_f);
  a50_fish1_m=mfexp(log_a50_fish4_m);
  a50_fish2=mfexp(log_a50_fish4_f );  //link because keeps bounding 
  ...
  delta_fish1_f=mfexp(log_delta_fish4_f);    // linked because only 5 years length comp data (1990-1995) to est historic sel
  delta_fish1_m=mfexp(log_delta_fish4_m);    // linked because only 5 years length comp data (1990-1995) to est historic sel
  delta_fish2=mfexp(log_delta_fish4_f);     // linked because only 5 years length comp data (1990-1995) to est historic sel
  ...
  for (j=1;j<=nages;j++)
   {
    ...
    if(fsh2_sel_opt==2) fish2_sel(j)=1/ (1+mfexp(-delta_fish2*(j-a50_fish2))); 
    ...
    if(fsh5_sel_opt==2)
     ...
     if(fsh5_sel_opt == 3) {
      fish5_sel_f(j)=(pow(j/a50_fish5_f,a50_fish5_f/(0.5*(sqrt(square(a50_fish5_f)+4*square(delta_fish5_f))-a50_fish5_f)))*mfexp((a50_fish5_f-j)/(0.5*(sqrt(square(a50_fish5_f)+4*square(delta_fish5_f))-a50_fish5_f))));
      fish5_sel_m(j)=(pow(j/a50_fish5_m,a50_fish5_m/(0.5*(sqrt(square(a50_fish5_m)+4*square(delta_fish5_m))-a50_fish5_m)))*mfexp((a50_fish5_m-j)/(0.5*(sqrt(square(a50_fish5_m)+4*square(delta_fish5_m))-a50_fish5_m))));
     }
...
// ###################################################################################################
     // all surveys have sex specific a50s
      ...
      delta_srv2_f=mfexp(log_delta_srv2_f);
      delta_srv2_m=mfexp(log_delta_srv2_m);    
      delta_srv10_f=mfexp(log_delta_srv10_f);
      delta_srv10_m=mfexp(log_delta_srv10_m);
  ...
  for (j=1;j<=nages;j++)
   {
    ...
    if(fsh1_sel_opt==3)
     { 
      fish1_sel_f(j) = (1/(1+mfexp(-delta_fish1_f*(j-a50_fish1_f))))*(1-(1/(1+mfexp(-gamma_fish1_f*(j-d50_fish1_f)))));
      fish1_sel_m(j) = (1/(1+mfexp(-delta_fish1_m*(j-a50_fish1_m))))*(1-(1/(1+mfexp(-gamma_fish1_m*(j-d50_fish1_m)))));
     }
    ...
    if(fsh2_sel_opt==5) fish2_sel(j)=1/ (1+mfexp(-delta_fish2*(j-a50_fish2)));
 ...
 for (j=1;j<=nages;j++)
  {
   ...
   if(srv1_sel_opt==3)
    { 
      srv1_sel_f(j) = (1/(1+mfexp(-delta_srv1_f*(j-a50_srv1_f))))*(1-(1/(1+mfexp(-gamma_srv1_f*(j-d50_srv1_f)))));
      srv1_sel_m(j) = (1/(1+mfexp(-delta_srv1_m*(j-a50_srv1_m))))*(1-(1/(1+mfexp(-gamma_srv1_m*(j-d50_srv1_m)))));
    }
 ...
 if(bias_ramp==1)
  {
     ...
     {
      b(y)=bmax*(1-((y-b_year_st)/(b_year_end-b_year_st)));
     }
     ...
     {
      b(y)=bmax*(1-((b_yr_end-y)/(endyr_rec_est-b_yr_end)));
     }
    }
 if(bias_ramp==0)
  {
...
// Start year
 ...
 switch (SrType)
  {
    ...
    if(rec_like_type==3)
     {
      natage_f(styr,1)=mfexp(log_Rztemp)*mfexp((-sigr*sigr)/2)*sexRatio(1);
      natage_m(styr,1)=mfexp(log_Rztemp)*mfexp((-sigr*sigr)/2)*sexRatio(2);
     }
    ...
    if(rec_like_type<3)
     ...
     if(sigma_R_early_switch==1 && styr<sigma_R_early_end)
      {
       natage_f(styr,1)=mfexp(log_mean_rec+log_rec_dev(styr)-b(styr)*sigma_R_early*sigma_R_early/2)*sexRatio(1);
       natage_m(styr,1)=mfexp(log_mean_rec+log_rec_dev(styr)-b(styr)*sigma_R_early*sigma_R_early/2)*sexRatio(2);       
      }
     else
      {
       natage_f(styr,1)=mfexp(log_mean_rec+log_rec_dev(styr)-b(styr)*sigr*sigr/2)*sexRatio(1);
       natage_m(styr,1)=mfexp(log_mean_rec+log_rec_dev(styr)-b(styr)*sigr*sigr/2)*sexRatio(2);
      }
    ...
    for (j=2;j<nages;j++)
     ...
     if(sigma_R_early_switch==1 && itmp<sigma_R_early_end)
      {
       natage_f(styr,j) =( mfexp(log_mean_rec  - (M(styr,j)+hist_hal_F*fish1_sel_f(j)) * double(j-1)+ log_rec_dev(itmp)-b(itmp)*sigma_R_early*sigma_R_early/2))*sexRatio(1); 
       natage_m(styr,j) = (mfexp(log_mean_rec  - (M(styr,j)+mdelta+hist_hal_F*fish1_sel_m(j)) * double(j-1)+ log_rec_dev(itmp)-b(itmp)*sigma_R_early*sigma_R_early/2))*sexRatio(2);      
      }
     else
      {
       natage_f(styr,j) =( mfexp(log_mean_rec  - (M(styr,j)+hist_hal_F*fish1_sel_f(j)) * double(j-1)+ log_rec_dev(itmp)-b(itmp)*sigr*sigr/2))*sexRatio(1); 
       natage_m(styr,j) = (mfexp(log_mean_rec  - (M(styr,j)+mdelta+hist_hal_F*fish1_sel_m(j)) * double(j-1)+ log_rec_dev(itmp)-b(itmp)*sigr*sigr/2))*sexRatio(2); 
     }
     }
     
    // ########## calc for plus group
      natage_f(styr,nages)      = (mfexp(log_mean_rec - (M(styr,nages-1)+hist_hal_F*fish1_sel_f(nages-1)) * (nages-1))/ (1. - exp(-(M(styr,nages-1)+hist_hal_F*fish1_sel_f(nages-1)))))*sexRatio(1);
      natage_m(styr,nages)      = (mfexp(log_mean_rec - (M(styr,nages-1)+mdelta+hist_hal_F*fish1_sel_m(nages-1)) * (nages-1))/ (1. - exp(-(M(styr,nages-1)+mdelta+hist_hal_F*fish1_sel_m(nages-1)))))*sexRatio(2);
     }
    break;
   }
  ...
  for ( i=styr;i <= endyr_rec;i++)
   {
     if(sigma_R_early_switch==1 && i<sigma_R_early_end)
      {
       natage_f(i,1)           = mfexp(log_rec_dev(i) + log_mean_rec-b(i)*sigma_R_early*sigma_R_early/2 )*sexRatio(1);  //fill in recruitment in current year
       natage_m(i,1)           = mfexp(log_rec_dev(i) + log_mean_rec-b(i)*sigma_R_early*sigma_R_early/2 )*sexRatio(2);      
      }
     if(sigma_R_early_switch==0 || i>=sigma_R_early_end)
      {
       natage_f(i,1)           = mfexp(log_rec_dev(i) + log_mean_rec-b(i)*sigr*sigr/2 )*sexRatio(1);  //fill in recruitment in current year
       natage_m(i,1)           = mfexp(log_rec_dev(i) + log_mean_rec-b(i)*sigr*sigr/2 )*sexRatio(2);
      }
 ...
 // FIll in abundance matrix for years with no rec_devs estimated, set recruit equal to Rave
   for(i=endyr_rec+1;i<endyr;i++)
    {
     natage_f(i,1)=mfexp(log_mean_rec-sigr*sigr/2)*sexRatio(1);
     natage_m(i,1)=mfexp(log_mean_rec-sigr*sigr/2)*sexRatio(2);
     sam_rec(i)            = natage_f(i,1)+natage_m(i,1); // OjO
  ...
  if(rec_like_type==1)
   {
     natage_f(endyr,1)=(mfexp(log_mean_rec+log_rec_dev(endyr)))*sexRatio(1);
     natage_m(endyr,1)=(mfexp(log_mean_rec+log_rec_dev(endyr)))*sexRatio(2);
   }
  else
   {
    natage_f(endyr,1)         = mfexp(log_mean_rec )*sexRatio(1); 
    natage_m(endyr,1)         = mfexp(log_mean_rec )*sexRatio(2);
   }
...
FUNCTION Get_Predicted_Values
 ...
 // ###############################################################################################################################################################################
  for (i=1;i<=nyrs_fish1_age;i++)
   {
    eac_fish1(i)  = (catage_fish1_f(yrs_fish1_age(i)) * ageage) & (catage_fish1_m(yrs_fish1_age(i)) * ageage);
 ...
 // ###############################################################################################################################################################################                                                           // Second Predicted Fishery size comps for 80s and 90s
  ...
  for (i=1;i<=nyrs_fish1_size;i++)
   {                  
    esc_fish1(i) =  ((catage_fish1_f(yrs_fish1_size(i))* size_age_f(yrs_fish1_size(i)))&(catage_fish1_m(yrs_fish1_size(i))* size_age_m(yrs_fish1_size(i))));   
    esc_fish1(i) /=sum(esc_fish1(i));
   }
   
  for (i=1;i<=nyrs_fish3_size;i++)
   {
    esc_fish3(i)  = (((catage_fish3_f(yrs_fish3_size(i)))* size_age_f(yrs_fish3_size(i)))&((catage_fish3_m(yrs_fish3_size(i)))* size_age_m(yrs_fish3_size(i))));                                              // Second Predicted Fishery size comps for 80s and 90s
    esc_fish3(i) /=sum(esc_fish3(i));
   } 
                                           // Second Predicted Fishery size comps for 80s and 90s
 ...
 // ###############################################################################################################################################################################
  for (i=1;i<=nyrs_srv1_age;i++)  // account for potential selectivity change in the DOM LL Survey
   {
    if(yrs_srv1_age(i)<yr_sel_chg_srv1)
     {   
      eac_srv1(i)  = ((((1-prop_m(yrs_srv1_age(i)))*elem_prod(srv1_sel_f,natage_f(yrs_srv1_age(i)))) * ageage)&((prop_m(yrs_srv1_age(i))*elem_prod(srv1_sel_m,natage_m(yrs_srv1_age(i)))) * ageage));                         // Predicted Survey age comps
    ...
    if(yrs_srv1_age(i)>=yr_sel_chg_srv1)
     {
      if(ph_LL_block2>0)  // if estimating new selectivity for recent time block then use srv10 selectivity
       {   
        eac_srv1(i)  = ((((1-prop_m(yrs_srv1_age(i)))*elem_prod(srv10_sel_f,natage_f(yrs_srv1_age(i))))*ageage)&((prop_m(yrs_srv1_age(i))*elem_prod(srv10_sel_m,natage_m(yrs_srv1_age(i)))) * ageage));                         // Predicted Survey age comps
        eac_srv1(i) /=sum(eac_srv1(i));
       } 
  ...
  for (i=1;i<=nyrs_srv2_age;i++)
   {
    eac_srv2(i)  = ((elem_prod(srv2_sel_f,natage_f(yrs_srv2_age(i)))*ageage)&(elem_prod(srv2_sel_m,natage_m(yrs_srv2_age(i)))*ageage));                        // Predicted Survey age comps
 ...
 // ###############################################################################################################################################################################                                                           // Second Predicted Fishery size comps for 80s and 90s
  for ( i=1;i<=nyrs_srv1_size;i++) //old length_age key
   {
    if(yrs_srv1_size(i)<yr_sel_chg_srv1)
     {   
      esc_srv1(i) = ((elem_prod(srv1_sel_f,natage_f(yrs_srv1_size(i))) * size_age_f(yrs_srv1_size(i)))&(elem_prod(srv1_sel_m,natage_m(yrs_srv1_size(i)))* size_age_m(yrs_srv1_size(i))));
      esc_srv1(i)  /=sum(esc_srv1(i));
     } 
    if(yrs_srv1_size(i)>=yr_sel_chg_srv1)
     {
      if(ph_LL_block2>0)  // if estimating new selectivity for recent time block then use srv10 selectivity
       {   
      esc_srv1(i) = ((elem_prod(srv10_sel_f,natage_f(yrs_srv1_size(i))) * size_age_f(yrs_srv1_size(i)))&(elem_prod(srv10_sel_m,natage_m(yrs_srv1_size(i)))* size_age_m(yrs_srv1_size(i))));
      esc_srv1(i)  /=sum(esc_srv1(i));
       } 
      if(ph_LL_block2<1) // if not estimating new selectivity for recent time block then use srv1 selectivity
       {   
  ...
  for ( i=1;i<=nyrs_srv2_size;i++)
   {
    esc_srv2(i)  = ((elem_prod(srv2_sel_f,natage_f(yrs_srv2_size(i)))*size_age_f(yrs_srv2_size(i)))&(elem_prod(srv2_sel_m,natage_m(yrs_srv2_size(i)))*size_age_m(yrs_srv2_size(i))));
    esc_srv2(i)  /=sum(esc_srv2(i)); 
   }
   
  for ( i=1;i<=nyrs_srv7_size;i++)
   {
    esc_srv7(i)  = ((elem_prod(srv7_sel_f,natage_f(yrs_srv7_size(i)))*size_age_f(yrs_srv7_size(i)))&(elem_prod(srv7_sel_m,natage_m(yrs_srv7_size(i)))* size_age_m(yrs_srv7_size(i))));        // Predicted Survey size comps (not used in POP model)
    esc_srv7(i)  /=sum(esc_srv7(i));
   }
...
// Calculate multinomial likelihoods for survey age, fishery size, and survey size and subtract "offset"
  ...
  for (i=1; i <= nyrs_fish1_age; i++) age_like(1) -= nsamples_fish1_age(i)*((oac_fish1(i) + 0.001) * log(eac_fish1(i) + 0.001)) ;
  for (i=1; i <= nyrs_srv1_age; i++)  age_like(2) -= nsamples_srv1_age(i)*((oac_srv1(i) + 0.001) * log(eac_srv1(i) + 0.001)) ;

  for (i=1; i <= nyrs_fish1_size; i++) age_like(3) -= nsamples_fish1_size(i)*((osc_fish1(i) + 0.001) * log(esc_fish1(i) + 0.001)) ;
  for (i=1; i <= nyrs_fish2_size; i++) age_like(5) -= nsamples_fish2_size(i)*((osc_fish2(i) + 0.001) * log(esc_fish2(i) + 0.001)) ;
  for (i=1; i <= nyrs_fish3_size; i++) age_like(6) -= nsamples_fish3_size(i)*((osc_fish3(i) + 0.001) * log(esc_fish3(i) + 0.001)) ;
  for (i=1; i <= nyrs_fish4_size; i++) age_like(8) -= nsamples_fish4_size(i)*((osc_fish4(i) + 0.001) * log(esc_fish4(i) + 0.001)) ;

  for (i=1; i <= nyrs_srv1_size; i++)  age_like(9) -= nsamples_srv1_size(i)*((osc_srv1(i) + 0.001) * log(esc_srv1(i) + 0.001)) ;
  for (i=1; i <= nyrs_srv2_size; i++)  age_like(11) -= nsamples_srv2_size(i)*((osc_srv2(i) + 0.001) * log(esc_srv2(i) + 0.001)) ;
  for (i=1; i <= nyrs_srv7_size; i++)  age_like(13) -= nsamples_srv7_size(i)*((osc_srv7(i) + 0.001) * log(esc_srv7(i) + 0.001)) ;
 ...
 if(data_reweight_switch==1) // data reweighting being done
  {
   ...
   age_like(3) *= wt_fish1_size_iter;  //1
   age_like(5) *= wt_fish2_size_iter; //
   age_like(6) *= wt_fish3_size_iter;  //1
   age_like(8) *= wt_fish4_size; //0
   age_like(9) *= wt_srv1_size_iter;  //1
   age_like(11) *= wt_srv2_size_iter;  //1
   age_like(13) *= wt_srv7_size_iter;  //1
   age_like(15) *= wt_srv7_age_iter;  //1
...
// Output file (tem.rep) which is loaded into tem.xls to display output
  ...
  report << wt_fish1_size_iter<<" "<<age_like(3)  <<" " ; report << "Fishery_Size_Composition_Likelihood_Fixed" << endl;
  report << wt_fish3_size_iter<<" "<<age_like(6) <<" " ; report << "Fishery_Size_Composition_Likelihood_Trawl" << endl;
  report << wt_srv1_size_iter<<" "<<age_like(9) <<" " ; report << "Survey_Size_Composition_Likelihood_Domestic LL" << endl;
  report << wt_srv2_size_iter<<" "<<age_like(11)  <<" " ; report << "Survey_Size_Composition_Likelihood_Cooperative_LL" << endl;
  report << wt_srv7_size_iter<<" "<<age_like(13)<<" " ; report << "Survey_Size_Composition_Likelihood_GOATrawl" << endl;
 ...
 */
  ...
  report << "Obs_P_fish1_size"<<len_bin_labels <<endl;
  for (i=1;i<=nyrs_fish1_size;i++)  report << yrs_fish1_size(i)<<" "<<osc_fish1(i)<<endl; report<<endl; 
  report << "Pred_P_fish1_size"<<len_bin_labels <<endl;
  for (i=1;i<=nyrs_fish1_size;i++) report << yrs_fish1_size(i)<<" "<<esc_fish1(i) <<endl; report<<endl;

  report << "Obs_P_fish3_size"<<len_bin_labels <<endl;
  for (i=1;i<=nyrs_fish3_size;i++)  report << yrs_fish3_size(i)<<" "<<osc_fish3(i)<<endl; report<<endl; 
  report << "Pred_P_fish3_size"<<len_bin_labels <<endl;
  for (i=1;i<=nyrs_fish3_size;i++) report << yrs_fish3_size(i)<<" "<<esc_fish3(i) <<endl; report<<endl;
  ...
  for (i=1;i<=nyrs_srv7_size;i++)  report << yrs_srv7_size(i)<<" "<<osc_srv7(i)<<endl; report<<endl; 
  report << "Pred_P_srv7_size"<<len_bin_labels <<endl;
  for (i=1;i<=nyrs_srv7_size;i++) report << yrs_srv7_size(i)<<" "<<esc_srv7(i) <<endl; report<<endl;

  report << "Obs_P_fish2_size"<<len_bin_labels <<endl;
  for (i=1;i<=nyrs_fish2_size;i++)  report << yrs_fish2_size(i)<<" "<<osc_fish2(i)<<endl; report<<endl; 
  report << "Pred_P_fish2_size"<<len_bin_labels <<endl;
  for (i=1;i<=nyrs_fish2_size;i++) report << yrs_fish2_size(i)<<" "<<esc_fish2(i) <<endl; report<<endl;

  report << "Obs_P_srv7_age"<<aa <<endl;
  for (i=1;i<=nyrs_srv7_age;i++)  report << yrs_srv7_age(i)<<" "<<oac_srv7(i)<<endl; report<<endl; 
  report << "Pred_P_srv7_age"<<len_bin_labels <<endl;
  for (i=1;i<=nyrs_srv7_age;i++) report << yrs_srv7_age(i)<<" "<<eac_srv7(i) <<endl; report<<endl;
...
FUNCTION write_sarareport
 ...
 sarareport << 0.875*B40 << "     # BMSY (kt)  is equilibrium spawning biomass at MSY (Tiers 1-2) or 7/8 x B40% (Tier 3)" << endl;
 ...
 sarareport<<"\"A full assessment was undertaken in "<<endyr<<" using  model 21.12.\""<<endl;
...
FUNCTION write_HQreport
 ...
 HQreport << "Sablefish -- Alaska wide " << endl;
 ...
 HQreport << 2  << endl;
  ...
  HQreport <<"#ASMT_MODEL_CATEGORY"<<endl;
   ...
   HQreport <<4<<endl;
   HQreport <<"#SIZE_AGE_COMP_INPUT_DATA"<<endl;
   HQreport <<4<<endl;
 ...
 HQreport<<"#FISHING_MORTALITY_ESTIMATES ----------------------------------------------------------------------------------------"<<endl;
   ...
   HQreport<<"F from 2022 asmt corresponding to specified 2021 OFL"<<endl;
   HQreport <<"#F_MSY"<<endl;
   HQreport<<FOFL<<endl;
 ...
 HQreport << "REPLACE WITH high 97.5% SSB FROM MCMC" <<endl;
   ...
   HQreport <<95<<endl;
    HQreport <<"#B_MSY"<<endl;
   HQreport << 0.875*B40<<endl;
    HQreport <<"#B_MSY_BASIS"<<endl;
   HQreport << "B35%"<<endl;

 HQreport<<"#TIME_SERIES_ESTIMATES ----------------------------------------------------------------------------------------------"<<endl;

 HQreport <<"#FISHERYYEAR - list years used in the model " << endl;
 HQreport << yy << endl;
  HQreport<<"#AGE - list of ages used in the model"<<endl;
 HQreport << aa << endl;
 HQreport <<"#RECRUITMENT - Number of recruits by year " << endl;
 HQreport  << pred_rec << endl;     
 HQreport <<"#SPAWNBIOMASS - Spawning biomass by year in kilo tons " << endl;
 HQreport  << spawn_biom << endl;  
 HQreport <<"#TOTALBIOMASS - Total biomass by year in kilo tons " << endl;
 HQreport  << tot_biom << endl;
 HQreport <<"#TOTFSHRYMORT - Fishing mortality rate by year " << endl;
   if((ph_ifq>0) && (ph_ifq_block2<1))  // post-IFQ sel does not  includes a recent time block
    {
 ...
 HQreport <<"#TOTALCATCH - Total catch by year in kilo tons " << endl;
 ...
 HQreport<<"\"A full assessment was undertaken in "<<endyr<<" using  model 21.12.\""<<endl;
