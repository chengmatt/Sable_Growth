...
// ###########################################################################################################################
 ...
 //############### for iterative reweighting runs
  init_number wt_fish1_age_female_iter           // Weight for fishery age compositions
  init_number wt_fish1_age_male_iter           // Weight for fishery age compositions

  init_number wt_srv1_age_female_iter           // Weight for survey age compositions
  init_number wt_srv1_age_male_iter           // Weight for survey age compositions

  init_number wt_srv2_age_female_iter           // Weight for survey age compositions
  init_number wt_srv2_age_male_iter           // Weight for survey age compositions
...
// Survey biomass estimates
 ...
 !! cout<<"Number of trawl years:"<<nyrs_srv7<<endl;
  ...
  init_matrix   oac_fish1_m(1,nyrs_fish1_age,1,nages)   // the actual year by year age comps
  init_matrix   oac_fish1_f(1,nyrs_fish1_age,1,nages)   // the actual year by year age comps
...
// Survey age composition data
  ...
  init_matrix   oac_srv1_f(1,nyrs_srv1_age,1,nages)   // the year by year age survey age comps
  init_matrix   oac_srv1_m(1,nyrs_srv1_age,1,nages)   // the year by year age survey age comps
  ...
  init_matrix   oac_srv2_m(1,nyrs_srv2_age,1,nages)   // the year by year age survey age comps
  init_matrix   oac_srv2_f(1,nyrs_srv2_age,1,nages)   // the year by year age survey age comps
...
// Initialize some counting variables
 ...
 int l
 int y
  vector offset(1,19);                                    // Multinomial "offset"
...
//   First step is to ensure that the data are expressed as proportions
  ...
  {
   oac_fish1_f(i)/=sum(oac_fish1_f(i));
   offset(1) -= nsamples_fish1_age(i) *((oac_fish1_f(i) + 0.001)*log(oac_fish1_f(i) + 0.001)); 
   oac_fish1_m(i)/=sum(oac_fish1_m(i));
   offset(17) -= nsamples_fish1_age(i) *((oac_fish1_m(i) + 0.001)*log(oac_fish1_m(i) + 0.001)); 
  }

  for (i=1; i<=nyrs_srv1_age; i++)
  {
   oac_srv1_f(i)/=sum(oac_srv1_f(i));
   offset(2) -= nsamples_srv1_age(i)*((oac_srv1_f(i) + 0.001)*log(oac_srv1_f(i) + 0.001));
   oac_srv1_m(i)/=sum(oac_srv1_m(i));
   offset(18) -= nsamples_srv1_age(i)*((oac_srv1_m(i) + 0.001)*log(oac_srv1_m(i) + 0.001));
  }
  ...
  {
   oac_srv2_f(i)/=sum(oac_srv2_f(i));
   offset(16) -= nsamples_srv2_age(i)*((oac_srv2_f(i) + 0.001)*log(oac_srv2_f(i) + 0.001));
   oac_srv2_m(i)/=sum(oac_srv2_m(i));
   offset(19) -= nsamples_srv2_age(i)*((oac_srv2_m(i) + 0.001)*log(oac_srv2_m(i) + 0.001));
  }
...
//###########################################################################################################
 ...
 // ########################################################################################################
  ...
  sdreport_vector                Fmort_fish1(styr,endyr);                        // Fishing mortality by year
  sdreport_vector                Fmort_fish3(styr,endyr);                        // Fishing mortality by year
...
// ########################################################################################################
 ...
 // ########################################################################################################
  ...
  init_bounded_number   log_delta_fish1_f(-5,5,ph_fish_sel_delt);                 // age between 50% selection and 95% selection....
  ...
  init_bounded_number       log_a50_fish2(-1,2.5,ph_fish_sel_delt_alt);                 // age at 50% selection                                                   
...
// ########################################################################################################
  ...
  init_bounded_number   log_delta_srv1_f(-5,4,ph_srv1_sel);                 // age between 50% selection and 95% selection....
  ...
  init_bounded_number   log_delta_srv1_m(-5,4,ph_srv1_sel);                 // age between 50% selection and 95% selection....
...
// Numbers at age and biomass
  ...
  sdreport_vector       tot_biom(styr,endyr);
...
// Catch at age
  ...
  matrix                eac_fish1_m(1,nyrs_fish1_age,1,nages)        // Expected proportion at age in fish
  matrix                eac_fish1_f(1,nyrs_fish1_age,1,nages)        // Expected proportion at age in fish  
  matrix                eac_srv1_m(1,nyrs_srv1_age,1,nages)        // Expected proportion at age in survey
  matrix                eac_srv1_f(1,nyrs_srv1_age,1,nages)        // Expected proportion at age in survey
  matrix                eac_srv2_m(1,nyrs_srv2_age,1,nages)        // Expected proportion at age in survey
  matrix                eac_srv2_f(1,nyrs_srv2_age,1,nages)        // Expected proportion at age in survey
  ...
  // Set up containers for sex ratios (calculating for females)
  // Expected
  vector               esra_fish1_f(1,nyrs_fish1_age);
  vector               esra_srv1_f(1,nyrs_srv1_age);
  vector               esra_srv2_f(1,nyrs_srv2_age);
  vector               esrs_fish1_f(1,nyrs_fish1_size);
  vector               esrs_fish2_f(1,nyrs_fish2_size);
  vector               esrs_fish3_f(1,nyrs_fish3_size);
  vector               esrs_srv1_f(1,nyrs_srv1_size);
  vector               esrs_srv2_f(1,nyrs_srv2_size);
  vector               esrs_srv7_f(1,nyrs_srv7_size);

  // Observed
  vector               osra_fish1_f(1,nyrs_fish1_age);
  vector               osra_srv1_f(1,nyrs_srv1_age);
  vector               osra_srv2_f(1,nyrs_srv2_age);
  vector               osrs_fish1_f(1,nyrs_fish1_size);
  vector               osrs_fish2_f(1,nyrs_fish2_size);
  vector               osrs_fish3_f(1,nyrs_fish3_size);
  vector               osrs_srv1_f(1,nyrs_srv1_size);
  vector               osrs_srv2_f(1,nyrs_srv2_size);
  vector               osrs_srv7_f(1,nyrs_srv7_size);
...
// Likelihoods and penalty functions
  ...
  vector        age_like(1,19);     // Likelihood values for age and size compositions allowance for up 6 comps
  vector        sel_like(1,12);     // LIkelihood values for selectivities with alowance for up to 6 selectivities
  vector        sexratio_like(1,9);     // Likelihood values for sex ratio linked to age and size comps
  ...
  for (j=1;j<=nages;j++)
   {
    ...
    if(fsh3_sel_opt==3) // Punt et. al 1996 gamma parameterization
     { 
      fish3_sel_f(j)=(pow(j/a50_fish3_f,a50_fish3_f/(0.5*(sqrt(square(a50_fish3_f)+4*square(delta_fish3_f))-a50_fish3_f)))*exp((a50_fish3_f-j)/(0.5*(sqrt(square(a50_fish3_f)+4*square(delta_fish3_f))-a50_fish3_f))));
      fish3_sel_m(j)=(pow(j/a50_fish3_m,a50_fish3_m/(0.5*(sqrt(square(a50_fish3_m)+4*square(delta_fish3_m))-a50_fish3_m)))*exp((a50_fish3_m-j)/(0.5*(sqrt(square(a50_fish3_m)+4*square(delta_fish3_m))-a50_fish3_m))));
     }
 ...
 // ###############################################################################################################################################################################
  ...
  for (i=styr;i<=endyr;i++)
   { 
    ...
    pred_srv7(i) = (q_srv7 *elem_prod(S_f_mid(i),natage_f(i))*elem_prod(srv7_sel_f,weight_f(i))+ q_srv7*elem_prod(S_m_mid(i),natage_m(i))*elem_prod(srv7_sel_m,weight_m(i)));   // Predicted Survey biomass
    pred_srv4(i) = (q_srv2 *(elem_prod(S_f_mid(i),natage_f(i))*srv2_sel_f)+q_srv2 * (elem_prod(S_m_mid(i),natage_m(i))*srv2_sel_m));
   }
   
  for (i=1979;i<=1989;i++)
   {
    pred_srv2(i) = (q_srv2 *(elem_prod(S_f_mid(i),natage_f(i))*elem_prod(srv2_sel_f,weight_f(i)))+q_srv2 *(elem_prod(S_m_mid(i),natage_m(i))*elem_prod(srv2_sel_m,weight_m(i)))); 
    pred_srv4(i) = (q_srv2 *(elem_prod(S_f_mid(i),natage_f(i))*srv2_sel_f)+q_srv2 * (elem_prod(S_m_mid(i),natage_m(i))*srv2_sel_m));
   } 
       
  for (i=1990;i<=1994;i++)
   {
    pred_srv2(i) = (q_srv9*(elem_prod(S_f_mid(i),natage_f(i))*elem_prod(srv2_sel_f,weight_f(i)))+q_srv9* (elem_prod(S_m_mid(i),natage_m(i))*elem_prod(srv2_sel_m,weight_m(i))));    // Predicted Survey biomass
    pred_srv4(i) = (q_srv9*(elem_prod(S_f_mid(i),natage_f(i))*srv2_sel_f)+q_srv9* (elem_prod(S_m_mid(i),natage_m(i))*srv2_sel_m));
   }   
  ...
  for (i=styr;i<=endyr;i++)
   {
    if(i<yr_sel_chg_srv1)
     {   
       pred_srv1(i) = (q_srv1 *(elem_prod(S_f_mid(i),natage_f(i))*elem_prod(srv1_sel_f,weight_f(i)))+q_srv1* (elem_prod(S_m_mid(i),natage_m(i))*elem_prod(srv1_sel_m,weight_m(i))));   // Predicted Survey biomass
       pred_srv3(i) = (q_srv1 *(elem_prod(S_f_mid(i),natage_f(i))*srv1_sel_f)+q_srv1 * (elem_prod(S_m_mid(i),natage_m(i))*srv1_sel_m));   // Predicted Survey biomass
     } 
    if(i>=yr_sel_chg_srv1)
     {
      if(ph_LL_block2>0 && ph_q_LL_srv_rec>0)  // if estimating new selectivity for recent time block then use srv10 selectivity AND NEW Q
       {   
        pred_srv1(i) = (q_LL_srvy_recent *(elem_prod(S_f_mid(i),natage_f(i))*elem_prod(srv10_sel_f,weight_f(i)))+q_LL_srvy_recent* (elem_prod(S_m_mid(i),natage_m(i))*elem_prod(srv10_sel_m,weight_m(i))));   // Predicted Survey biomass
        pred_srv3(i) = (q_LL_srvy_recent *(elem_prod(S_f_mid(i),natage_f(i))*srv10_sel_f)+q_LL_srvy_recent * (elem_prod(S_m_mid(i),natage_m(i))*srv10_sel_m));   // Predicted Survey biomass
       }
      if(ph_LL_block2>0 && ph_q_LL_srv_rec<1)  // if estimating new selectivity for recent time block then use srv10 selectivity AND old Q
       {   
        pred_srv1(i) = (q_srv1 *(elem_prod(S_f_mid(i),natage_f(i))*elem_prod(srv10_sel_f,weight_f(i)))+q_srv1* (elem_prod(S_m_mid(i),natage_m(i))*elem_prod(srv10_sel_m,weight_m(i))));   // Predicted Survey biomass
        pred_srv3(i) = (q_srv1 *(elem_prod(S_f_mid(i),natage_f(i))*srv10_sel_f)+q_srv1 * (elem_prod(S_m_mid(i),natage_m(i))*srv10_sel_m));   // Predicted Survey biomass
       } 
      ...
      if(ph_LL_block2<1 && ph_q_LL_srv_rec>0) // if not estimating new selectivity for recent time block then use srv1 selectivity
       {   
        pred_srv1(i) =(q_LL_srvy_recent *(elem_prod(S_f_mid(i),natage_f(i))*elem_prod(srv1_sel_f,weight_f(i)))+q_LL_srvy_recent* (elem_prod(S_m_mid(i),natage_m(i))*elem_prod(srv1_sel_m,weight_m(i))));   // Predicted Survey biomass
        pred_srv3(i) =(q_LL_srvy_recent *(elem_prod(S_f_mid(i),natage_f(i))*srv1_sel_f)+q_LL_srvy_recent * (elem_prod(S_m_mid(i),natage_m(i))*srv1_sel_m));   // Predicted Survey biomass
       }
     }
   }
 ...
 // ###############################################################################################################################################################################                                                           // Second Predicted Fishery size comps for 80s and 90s
  ...
  for (i=1;i<=nyrs_fish2_size;i++)
   {                     // Lets you use a second matrix for part of it
    esc_fish2(i)  = (elem_prod(fish2_sel,natage_f(yrs_fish2_size(i)))+elem_prod(fish2_sel,natage_m(yrs_fish2_size(i))))* sizeage_all;                          // Predicted Survey age comps
...
// Calculate multinomial likelihoods for survey age, fishery size, and survey size and subtract "offset"
  ...
  for (i=1; i <= nyrs_fish1_age; i++) {
    age_like(1) -= nsamples_fish1_age(i)*((oac_fish1_f(i) + 0.001) * log(eac_fish1_f(i) + 0.001)) ;
    age_like(17) -= nsamples_fish1_age(i)*((oac_fish1_m(i) + 0.001) * log(eac_fish1_m(i) + 0.001)) ;
  }

  for (i=1; i <= nyrs_srv1_age; i++) {
    age_like(2) -= nsamples_srv1_age(i)*((oac_srv1_f(i) + 0.001) * log(eac_srv1_f(i) + 0.001)) ;
    age_like(18) -= nsamples_srv1_age(i)*((oac_srv1_m(i) + 0.001) * log(eac_srv1_m(i) + 0.001)) ;
  }
  ...
  for (i=1; i <= nyrs_srv2_age; i++)   age_like(16) -= nsamples_srv2_age(i)*((oac_srv2_f(i) + 0.001) * log(eac_srv2_f(i) + 0.001)) ;
  for (i=1; i <= nyrs_srv2_age; i++)   age_like(19) -= nsamples_srv2_age(i)*((oac_srv2_m(i) + 0.001) * log(eac_srv2_m(i) + 0.001)) ;
 ...
 if(data_reweight_switch==0) // no reweighting being done
  {
   ...
   age_like(17) *= wt_fish1_age;    //1               // Multiple each likelihood by their weights from .ctl file
   age_like(2) *= wt_srv1_age;  //1
   age_like(18) *= wt_srv1_age;  //1
   ...
   age_like(19) *= wt_srv1_age;  //1
 }

 if(data_reweight_switch==1) // data reweighting being done
  {
   age_like(1) *= wt_fish1_age_female_iter;    //1               // Multiple each likelihood by their weights from .ctl file
   age_like(17) *= wt_fish1_age_male_iter;    //1               // Multiple each likelihood by their weights from .ctl file
   age_like(2) *= wt_srv1_age_female_iter;  //1
   age_like(18) *= wt_srv1_age_male_iter;  //1
   ...
   age_like(15) *= wt_srv7_age;  //1
   age_like(16) *= wt_srv2_age_female_iter;  //1
   age_like(19) *= wt_srv2_age_male_iter;  //1
...
// Output file (tem.rep) which is loaded into tem.xls to display output
  ...
  report << wt_fish1_age_female_iter <<" "<<age_like(1)  <<" " ; report << "Fishery_Age_Female_Composition_Likelihood" << endl;
  report << wt_fish1_age_male_iter <<" "<<age_like(17)  <<" " ; report << "Fishery_Age_Male_Composition_Likelihood" << endl;
  report << wt_srv1_age_female_iter <<" "<<age_like(2)  <<" " ; report << "Survey_Age_Female_Composition_Likelihood_DomesticLL" << endl;
  report << wt_srv1_age_male_iter <<" "<<age_like(18)  <<" " ; report << "Survey_Age_Male_Composition_Likelihood_DomesticLL" << endl;
  report << wt_srv2_age_female_iter <<" "<<age_like(16)  <<" " ; report << "Survey_Age_Female_Composition_Likelihood_CooperativeLL" << endl;
  report << wt_srv2_age_male_iter <<" "<<age_like(19)  <<" " ; report << "Survey_Age_Male_Composition_Likelihood_CooperativeLL" << endl;
 ...
 */
  ...
  report << "Obs_P_fish_age Female"<<aa <<endl;
  for (i=1;i<=nyrs_fish1_age;i++) report << yrs_fish1_age(i)<<" "<<oac_fish1_f(i)<<endl; report<<endl;
  report << "Pred_P_fish1_age Female"<<aa <<endl;
  for (i=1;i<=nyrs_fish1_age;i++) report << yrs_fish1_age(i)<<" "<<eac_fish1_f(i) <<endl; report<<endl;

  report << "Obs_P_fish_age Male"<<aa <<endl;
  for (i=1;i<=nyrs_fish1_age;i++) report << yrs_fish1_age(i)<<" "<<oac_fish1_m(i)<<endl; report<<endl;
  report << "Pred_P_fish1_age Male"<<aa <<endl;
  for (i=1;i<=nyrs_fish1_age;i++) report << yrs_fish1_age(i)<<" "<<eac_fish1_m(i) <<endl; report<<endl;
  ...
  report << "Obs_P_srv1_age Female"<<aa <<endl;
  for (i=1;i<=nyrs_srv1_age;i++)  report << yrs_srv1_age(i)<<" "<<oac_srv1_f(i)<<endl; report<<endl; 
  report << "Pred_P_srv1_age Female"<<aa <<endl;
  for (i=1;i<=nyrs_srv1_age;i++) report << yrs_srv1_age(i)<<" "<<eac_srv1_f(i) <<endl; report<<endl;

  report << "Obs_P_srv1_age Male"<<aa <<endl;
  for (i=1;i<=nyrs_srv1_age;i++)  report << yrs_srv1_age(i)<<" "<<oac_srv1_m(i)<<endl; report<<endl; 
  report << "Pred_P_srv1_age Male"<<aa <<endl;
  for (i=1;i<=nyrs_srv1_age;i++) report << yrs_srv1_age(i)<<" "<<eac_srv1_m(i) <<endl; report<<endl;

  report << "Obs_P_srv2_age Female"<<aa <<endl;
  for (i=1;i<=nyrs_srv2_age;i++)   report << yrs_srv2_age(i)<<" "<<oac_srv2_f(i)<<endl; report<<endl; 
  report << "Pred_P_srv2_age Female"<<aa <<endl;
  for (i=1;i<=nyrs_srv2_age;i++) report << yrs_srv2_age(i)<<" "<<eac_srv2_f(i) <<endl; report<<endl;

  report << "Obs_P_srv2_age Male"<<aa <<endl;
  for (i=1;i<=nyrs_srv2_age;i++)   report << yrs_srv2_age(i)<<" "<<oac_srv2_m(i)<<endl; report<<endl; 
  report << "Pred_P_srv2_age Male"<<aa <<endl;
  for (i=1;i<=nyrs_srv2_age;i++) report << yrs_srv2_age(i)<<" "<<eac_srv2_m(i) <<endl; report<<endl;
 ...
 // eport << yrs_srv2_size(i)<<" "<<osc_srv2_m(i) 
...
// for (i=1;i<=nyrs_srv2_size;i++) {
//       sdnr_srv2_size+=sdnr(esc_srv2_m(i),osc_srv2_m(i),wt_srv2_size*double(nsamples_srv2_size(i)))/nyrs_srv2_size/2;
// ullrep << yrs_srv2_size(i)<<" "<<osc_srv2_m(i) 
//     <<" eff_N "<<(1-esc_srv2_m(i))*esc_srv2_m(i)/norm2(osc_srv2_m(i)-esc_srv2_m(i)) <<" N "<<nsamples_srv2_size(i)
